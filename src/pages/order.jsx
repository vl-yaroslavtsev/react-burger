...
export function OrderPage() {
  ...
  if (error) {
    return <section className={cn(styles.container, "mt-2 mb-2 text text_type_main-default")}>
      <p className={styles.error}>Не удалось найти заказ {id}.<br /> {error}</p>
    </section>
  }
  ...
  if (loading) {
    return <section className={cn(styles.container, "mt-2 mb-2")}>
      <div className={styles.center}>
        <Skeleton width={200} className={"text text_type_digits-default mb-10"} />
      </div>
  ...
  return order && (
    <section className={cn(styles.container, "mt-2 mb-2")}>
      <h2 className={cn(styles.center, "text text_type_digits-default mb-10")}>
        #{order.number}
      </h2>
